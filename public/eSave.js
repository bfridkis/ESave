    ...
    req.addEventListener("load", () => {
      if(req.status >= 200 && req.status < 400){
       ...
       let results = JSON.parse(req.responseText);
        ...
        let initPriHeader = initNameHeaderRow.appendChild(document.createElement("th"));
        initNameHeader.textContent = "TOTAL BEFORE SHIPPING AND PROMOTIONS";
        initNameHeader.style.textDecoration = "underline";
        let initNameRow = stageTable.appendChild(document.createElement("tr"));
        let initName = initNameRow.appendChild(document.createElement("td"));
        let initPriBD = initNameRow.appendChild(document.createElement("td"));
        initPriBD.style.fontSize = "1rem";
        initPriBD.style.textDecoration = "italic";
        initPriBD.textContent = "( " + qts[0].textContent + " x " + results[0]["PRICE_PER_UNIT"] + " )";
        initPriName.textContent = results[0]["INITIAL_PRICE"];
