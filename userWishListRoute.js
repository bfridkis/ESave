module.exports = (app) => {
  ...
  function getWishList(res, mysql, context, userid){
      ...
      mysql.pool.query("SELECT (@rownum := @rownum + 1) AS row_number, z.* " +
          "FROM (SELECT wish_list.order, order.current_price, DATE_FORMAT(order.created_on, '%W, %M %e %Y, %I:%i %p') AS created_on, order_product.quantity, product.name AS product, promotion.description AS promotion, retailer.name AS retailer FROM wish_list " +
